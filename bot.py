...
class VIEBot(discord.Client):
    ...
    async def check_vie(self):
        ...
        # --- TEST DYNAMIQUE AU DÉMARRAGE ---
        try:
            async with aiohttp.ClientSession() as test_session:
                found_ids = await self._extract_offer_ids(test_session)
                if found_ids:
                    latest_id = found_ids[0]
                    details = await self._fetch_offer_details(test_session, latest_id)
                    await channel.send(embed=self._build_offer_embed(details, latest_id, is_test=True))
                    print(f"✉️ Test envoyé : offre #{latest_id}")
